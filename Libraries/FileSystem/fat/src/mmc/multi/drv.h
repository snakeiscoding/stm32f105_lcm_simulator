...
void *spi_add_device (unsigned long driver_param);	/* add new device */
int spi_del_device (void *);	/* delete device */
int spi_check_device (void *);	/* check current subdevice state */
t_mmc_dsc *get_mmc_dsc (void);	/* get current drives mmc descriptor */
F_DRIVER *get_mmc_driver (void);/* get current drives driver structure */
