 ...
 *
 ***************************************************************************/
static unsigned char mmc_cmd (t_mmc_dsc *mmc_dsc, unsigned long cmd, unsigned long data)
{
   register int a;
...
static int mmc_read(F_DRIVER *driver,unsigned char *dwaddr,unsigned long sector,int cnt)
{
   ...
   if (spi_check_device(driver->user_ptr)) return MMC_ERR_NOTINITIALIZED; /* card is not initialized */
   ...
   {
     r1=mmc_cmd(mmc_dsc,CMD_SET_BLOCK_COUNT,cnt);
     if (r1)
     {
       SPI_CS_HI;
   ...
   sector<<=9;
#endif
   r1=mmc_cmd(mmc_dsc,multi?CMD_READ_MULTIPLE_BLOCK:CMD_READ_SINGLE_BLOCK,sector);
   if (r1)
   {
     SPI_CS_HI;
     if (r1&BIT_IDLE_STATE) mmc_dsc->initok=0;
   ...
   {
     if (mmc_cmd(mmc_dsc,CMD_STOP_TRANS,0)) (void)spiWaitBusy();
   }
...
static int mmc_write(F_DRIVER *driver,unsigned char *dwaddr,unsigned long sector,int cnt)
{
   ...
   sector<<=9;
#endif
   r1=mmc_cmd(mmc_dsc,multi?CMD_WRITE_MULTIPLE_BLOCK:CMD_WRITE_SINGLE_BLOCK,sector);
   if (r1)
   {
     SPI_CS_HI;
     if (r1&BIT_IDLE_STATE) mmc_dsc->initok=0;
...
static int mmc_getcsd (t_mmc_dsc *mmc_dsc)
{
  unsigned char a;
  unsigned char *csd=mmc_dsc->CSD;

  SPI_CS_LO;
  if (mmc_cmd(mmc_dsc,CMD_SEND_CSD,0))
  {
    SPI_CS_HI;
 ...
 *
 ***************************************************************************/
static int mmc_initcard(t_mmc_dsc *mmc_dsc)
{
   long a;
   long retry;
   int ret;
   ...
   {
      SPI_CS_LO;
      if (mmc_cmd(mmc_dsc,CMD_GO_IDLE_STATE,0)==BIT_IDLE_STATE) break;
      SPI_CS_HI;
   }
   SPI_CS_HI;
...
#if SDHC_ENABLE
   SPI_CS_LO;
   r1=mmc_cmd(mmc_dsc,CMD_SEND_IF_COND,0x1a5);
   a=spi_rx1(); 
   a<<=8;
   a|=spi_rx1(); 
   a<<=8;
   ...
   SPI_CS_LO;
   r1=mmc_cmd(mmc_dsc,CMD_APP_CMD,0);
   SPI_CS_HI;
   if (r1==BIT_IDLE_STATE)
   {
     SPI_CS_LO;
     r1=mmc_cmd(mmc_dsc,ACMD_SEND_OP_COND,(t&_T_SDV2)?0x40000000:0);
     SPI_CS_HI;
   }
   ...
   for (retry=0,r1=BIT_IDLE_STATE;retry<5000 && r1;retry++)
   {
     ...
     {
       SPI_CS_LO;
       r1=mmc_cmd(mmc_dsc,CMD_SEND_OP_COND,0);
       SPI_CS_HI;
     }
     else
     {
       ...
       SPI_CS_HI;
       SPI_CS_LO;
       r1=mmc_cmd(mmc_dsc,ACMD_SEND_OP_COND,a);
       SPI_CS_HI;
     }
   }
   ...
   /* read OCR */
   SPI_CS_LO;
   if (mmc_cmd(mmc_dsc,CMD_READ_OCR,0))
   {
     SPI_CS_HI;
     return MMC_ERR_CMD;
   }
   ...
   SPI_CS_LO;
   if (mmc_cmd(mmc_dsc,CMD_SET_BLOCKLEN,512))
   ...
   SPI_CS_LO;
   if (mmc_cmd(mmc_dsc,CMD_CRC_ON_OFF,USE_CRC))
   {  /* set crc on/off */
     SPI_CS_HI;
     return MMC_ERR_CMD;
   }
   ...
   ret=mmc_getcsd(mmc_dsc);
   if (ret) return ret ;

   SPI_CS_LO;
   r1=mmc_cmd(mmc_dsc,CMD_SET_BLOCK_COUNT,1);
   SPI_CS_HI;
...
static long mmc_getstatus(F_DRIVER *driver)
{
   ...
   else
   {
      if (mmc_dsc->initok)
      {
        unsigned short rc;
        SPI_CS_LO;
        rc=mmc_cmd(mmc_dsc,CMD_SEND_STATUS,0);
        rc<<=8;
        rc|=spi_rx1();
        SPI_CS_HI;
...
{
   (void)spi_del_device(driver->user_ptr);
}
...
F_DRIVER *mmc_initfunc(unsigned long driver_param)
{
   ...
   (void)spi_init();
	...
	user_ptr=spi_add_device(driver_param);
	if (user_ptr)
	{
		(void)spi_check_device(user_ptr);

		mmc_dsc=get_mmc_dsc();
		(void)_memset(mmc_dsc,0,sizeof(t_mmc_dsc));

		driver=get_mmc_driver();
		driver->readsector=mmc_readsector;
   		driver->writesector=mmc_writesector;
   		driver->readmultiplesector=mmc_readmultiplesector;
   		driver->writemultiplesector=mmc_writemultiplesector;
   		driver->getstatus=mmc_getstatus;
   		driver->getphy=mmc_getphy;
   		driver->release=mmc_release;
   		driver->user_ptr=user_ptr;
	}
