...
#if F_DELETE_CONTENT
extern int _f_removechainandcontent(F_VOLUME *vi,unsigned long cluster);
#endif
...
extern int _f_findopensize(unsigned long *ofsize,int drivenum,F_POS *pos);
extern int _f_ioctl_endofdelete(F_VOLUME *vi);
extern int _f_ioctl_multiplesectorerase(F_VOLUME *vi,unsigned long sector, unsigned long sectorcou, void *buffer);

#if (!F_LONGFILENAME)
extern int _f_createfullname(char *buffer,int buffersize,int drivenum,char *path,char *filename,char *fileext);
#else
extern int _f_createfullname(char *buffer,int buffersize,int drivenum,char *path,char *lname);
#endif
